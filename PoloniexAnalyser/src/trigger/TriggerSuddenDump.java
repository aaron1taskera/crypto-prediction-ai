...
public class TriggerSuddenDump extends Trigger {

	public TriggerSuddenDump() throws Exception { super("suddendump", Trigger.HALF_HOUR); }

	public TriggerSuddenDump(String name) throws Exception { super(name, Trigger.HALF_HOUR); }
	...
	public boolean isTriggered(Candlestick[] candles, Candlestick[] btcCandles, int index) {
		if (Numerics.pc(candles[index].getOpen(), candles[index].getClose()) < Candlestick.DUMP_DELTA) {
			try {
				return true;
	...
	public void _loadMax(DataSet ds) throws Exception {
